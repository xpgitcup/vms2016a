	...
	<body>
		...
		<div id="show-block" class="content scaffold-show" role="main">
			...
			<ol class="property-list block">
				...
				<g:if test="${blockInstance?.name}">
				<li class="fieldcontain">
					<span id="name-label" class="property-label"><g:message code="block.name.label" default="Name" /></span>
					
						<span class="property-value" aria-labelledby="name-label"><g:fieldValue bean="${blockInstance}" field="name"/></span>
					
				</li>
				</g:if>
			
				<g:if test="${blockInstance?.well}">
				<li class="fieldcontain">
					<span id="well-label" class="property-label"><g:message code="block.well.label" default="Well" /></span>
					
						<g:each in="${blockInstance.well}" var="w">
						<span class="property-value" aria-labelledby="well-label"><g:link controller="well" action="show" id="${w.id}">${w?.encodeAsHTML()}</g:link></span>
						</g:each>
